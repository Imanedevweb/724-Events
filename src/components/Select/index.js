...
const Select = ({
  ...
  const changeValue = (newValue) => { 
    ...
    onChange(newValue);  // Ajout newValue comme paramètre de onChange   // Transmettre la nouvelle valeur sélectionnée à la fonction parente
    setValue(newValue);
      ...
      <div className="Select">
        <ul>
          <li className={collapsed ? "SelectTitle--show" : "SelectTitle--hide"}
            title={value || (!titleEmpty ? "Toutes les options" : "")}
            >
        ...
        <button
          ...
          title={collapsed ? "ouvrir la sélection" : "Fermer la sélection"}
        >
          <Arrow />
        </button>
      </div>
